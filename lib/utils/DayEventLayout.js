...
function () {
  ...
  (0, _createClass2.default)(Event, [{
    ...
    get: function get() {
      ...
      return (this.row && this.row._width) || 100;
    }
    /**
    ...
    get: function get() {
      ...
      var overlap = Math.min(100, this._width * 2); // Containers can always grow.
      ...
      var leaves = (this.row && this.row.leaves) || [];
    ...
    get: function get() {
      ...
      var _this$row = this.row,
          leaves = (_this$row && _this$row.leaves) || [],
          xOffset = (_this$row && _this$row.xOffset) || 0,
          _width = (_this$row && _this$row._width) || 0;
...
function sortByRender(events) {
  ...
  while (sortedByTime.length > 0) {
    ...
    for (var i = 0; i < sortedByTime.length; i++) {
      ...
      if (event.data.title !== 'GRIDHOURS' && event.endMs > test.startMs) continue; // We've found the first event of the next event group.
...
function getStyledEvents(_ref2) {
  ...
  var _loop = function _loop(i) {
    ...
    if (row && event.data.title !== 'GRIDHOURS') {
      // Found a row, so add it.
      row.leaves.push(event);
      event.row = row;
    } else if(!row && event.data.title !== 'GRIDHOURS'){
